...
type BotkitMessageTemplateAsync = (
  activity: Activity,
  values: any,
) => Promise<Partial<BotkitMessageTemplate>> | Promise<string>
...
export class BotkitConversation<O extends object = {}> extends Dialog<O> {
  ...
  public say(
    message:
      | Partial<BotkitMessageTemplate>
      | BotkitMessageTemplateAsync
      | string,
  ): BotkitConversation {
  ...
  public ask(
    message:
      ...
      | BotkitMessageTemplateAsync
      | string,
    handlers: BotkitConvoTrigger | BotkitConvoTrigger[],
  ...
  public addQuestion(
    ...
    let message = msg as ((Partial<BotkitMessageTemplate> | BotkitMessageTemplateAsync & {collect: string} | string) )
    if (!thread_name) {
      thread_name = 'default';
    }
    ...
    message.collect.options.forEach(o => {
      if (!o.type) {
        o.type = 'string';
      }
    });
  ...
  public onChange(
    variable: string,
    handler: (response, convo, bot) => Promise<any>,
  ): void {
  ...
  public async beginDialog(dc: DialogContext, options: any): Promise<any> {
    const { botWorker, ...restOptions } = options
    delete options.botWorker;
    // Initialize the state
    const state = dc.activeDialog.state;
    state.options = restOptions || {};
    state.values = { ...options };

     // @ts-ignore
     dc.botWorker = botWorker;

    // Run the first step
    return await this.runStep(
      dc,
      0,
      state.options.thread || 'default',
      DialogReason.beginCalled,
    );
  }
  ...
  public async continueDialog(dc: DialogContext): Promise<any> {
    ...
    return await this.resumeDialog(
      dc,
  ...
  public async resumeDialog(dc, reason, result): Promise<any> {
    ...
    return await this.runStep(
      dc,
      state.stepIndex + 1,
      state.thread || 'default',
      reason,
      result,
    );
  }

  private async typingIndicator(dc, line, step, activity) {
    let typingDelay = this._controller.adapter.typingDelay || 1500;
    if (line && line.channelData && line.channelData.typingDelay) {
  ...
  private async onStep(dc, step): Promise<any> {
    ...
    if (step.result && previous && previous.collect) {
      ...
      if (previous.collect.options) {
        var paths = previous.collect.options.filter(option => {
          return !option.default === true;
        });
        var default_path = previous.collect.options.filter(option => {
          return option.default === true;
        })[0];
        var path = null;
      ...
      if (line.collect && line.action !== 'beginDialog') {
        ...
        try {
          return await dc.prompt(
            this._prompt,
            await this.makeOutgoing(dc, line, step.values),
          );
        } catch (err) {
          console.error(err);
          await dc.context.sendActivity(
            `Failed to start prompt ${this._prompt}`,
          );
        ...
        if (
          ...
          line.text ||
          line.attachments ||
          (line.channelData && Object.keys(line.channelData).length)
        ) {
          await this.typingIndicator(dc, line, step, activity)
  ...
  private async runStep(
    ...
    const step = {
      ...
      next: async stepResult => {
        if (nextCalled) {
          throw new Error(
            `ScriptedStepContext.next(): method already called for dialog and step '${
              this.id
            }[${index}]'.`,
          );
        }
  ...
  private async makeOutgoing(
    ...
    if (line.quick_replies && typeof line.quick_replies != 'function') {
      outgoing = MessageFactory.suggestedActions(
        line.quick_replies.map(reply => {
          return {
    ...
    if (typeof line.quick_replies == 'function') {
      ...
      outgoing.suggestedActions = {
        actions: outgoing.channelData.quick_replies.map(reply => {
          return {
            type: ActionTypes.PostBack,
            title: reply.title,
            text: reply.payload,
    ...
    if (typeof line.attachments == 'function') {
      ...
      outgoing.attachments = outgoing.channelData.attachments = await line.attachments(
        line,
        vars,
      );
    }
    ...
    if (line.attachments && typeof line.attachments != 'function') {
      outgoing.attachments = this.parseTemplatesRecursive(
        line.attachments,
        vars,
      );
    }
    ...
    if (outgoing.channelData.attachments) {
      outgoing.channelData.attachments = this.parseTemplatesRecursive(
        outgoing.channelData.attachments,
        vars,
      );
    }
    if (outgoing.channelData.blocks) {
      outgoing.channelData.blocks = this.parseTemplatesRecursive(
        outgoing.channelData.blocks,
        vars,
      );
    }
    ...
    if (outgoing.channelData.attachment) {
      outgoing.channelData.attachment = this.parseTemplatesRecursive(
    ...
    if (outgoing.channelData.quick_replies) {
      outgoing.channelData.quick_replies = this.parseTemplatesRecursive(
        outgoing.channelData.quick_replies,
        vars,
      );
    }
    // process templates in quick replies
    if (outgoing.suggestedActions) {
      outgoing.suggestedActions = this.parseTemplatesRecursive(
        outgoing.suggestedActions,
        vars,
      );
    }
    ...
    return new Promise(async (resolve, reject) => {
      ...
      that._controller.middleware.send.run(
        bot,
        outgoing,
        (err, bot, outgoing) => {
          if (err) {
      ...
      for (let a = 0; a < attachments.length; a++) {
        for (let key in attachments[a]) {
          if (typeof attachments[a][key] === 'string') {
            attachments[a][key] = mustache.render(attachments[a][key], {
              vars: vars,
            });
          } else {
            attachments[a][key] = this.parseTemplatesRecursive(
              attachments[a][key],
              vars,
            );
          }
   ...
   * @param step The current step object
   */
  private async gotoThreadAction(
    thread: string,
    dc: DialogContext,
    step: BotkitConversationStep,
  ): Promise<any> {
