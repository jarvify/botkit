...
export class Botkit {
  ...
  public constructor(config: BotkitConfiguration) {
    ...
    if (this._config.disable_webserver !== true) {
      if (!this._config.webserver) {
        ...
        this.http.listen(process.env.port || process.env.PORT || 3000, () => {
          if (this._config.disable_console !== true) {
            console.log(
              `Webhook endpoint online:  http://localhost:${process.env.PORT || 3000}${this._config.webhook_uri}`
            );
          }
    ...
    if (!this._config.adapter) {
      ...
      if (this.webserver) {
        if (this._config.disable_console !== true) {
          console.log(
            `Open this bot in Bot Framework Emulator: bfemulator://livechat.open?botUrl=` +
              encodeURIComponent(`http://localhost:${process.env.PORT || 3000}${this._config.webhook_uri}`)
          );
        }
      }
    } else {
  ...
  public async handleTurn(turnContext: TurnContext): Promise<any> {
    ...
    const message: BotkitMessage = {
      ...
      type:
        turnContext.activity.channelData && turnContext.activity.channelData.botkitEventType
          ? turnContext.activity.channelData.botkitEventType
